...
const getFromNameQuery = "select family,source_json,popularity,kind,username,comment_text,comment_id,public.user.user_id, AVG(rating.rating) "+
    ...
    getFromFontIdQuery ="select username,comment_text,comment.comment_id,public.user.user_id, AVG(rating.rating) as \"rating\", count(rating.rating) as \"total votes\" "+
        "from public.comment join public.font using(font_id) join public.user using(user_id) left join rating ON comment.comment_id = rating.comment_id where public.font.font_id = $1 group by comment.comment_id, public.user.user_id;",
